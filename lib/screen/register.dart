...
class _RegisterState extends State<Register> {
  ...
  Future<Null> processInsertMySQL() async {
    ...
    Future<bool> checkDuplicateEmail(String email) async {
      String apiCheckDuplicateEmail =
          '${MyCostant.domain}/customer_checkemail.php?isAdd=true&cust_email=$email';
      bool isDuplicate = false;
      await Dio().get(apiCheckDuplicateEmail).then((value) {
    ...
    bool isDuplicate = await checkDuplicateEmail(cust_email);
    if (isDuplicate) {
      MyDialog().normalDialog(
          context, 'สมัครสมาชิกไม่สำเร็จ !!', 'อีเมลที่คุณกรอกมีในระบบแล้ว');
    } else {
      String apiInsertUser =
          '${MyCostant.domain}/customer_insertCustomer.php?isAdd=true&cust_firstname=$cust_firstname&cust_lastname=$cust_lastname&cust_phone=$cust_phone&cust_email=$cust_email&password=$password&wallet=0&payment=0';
      await Dio().get(apiInsertUser).then((value) {
        if (value.toString() == 'true') {
          ...
          MyDialog().normalDialog(
              context, 'สมัครสมาชิกไม่สำเร็จ', 'กรุณาลองใหม่อีกครั้ง');
        }
      });
    }
  }
